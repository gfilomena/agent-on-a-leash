...
import type { CheckResult, Purchase } from "@/lib/types";
      ...
      <DrawerContent className="glass-strong !max-h-[85%] rounded-t-[28px] border-x-0 border-b-0 text-foreground">
        {d && (
          <div className="overflow-y-auto px-5 pb-8 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <DrawerHeader className="px-0 pt-4 !text-left">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="amount text-[13px] text-muted-foreground">
                  {clock(d.at)} · decided in {seconds(Math.max(d.answerMs, d.decidedInMs))}
                </span>
              </div>
            ...
            {d.answeredBy === "you" && d.reviewReason && (
              <p className="mb-4 rounded-2xl bg-white/[0.04] p-3.5 text-[13.5px] text-muted-foreground">Compass asked you because: {d.reviewReason.replace(/^Needs review: /, "")}</p>
            )}
            ...
            <div className="rounded-2xl bg-white/[0.04] p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  {d.items.map((item, i) => (
                    <div key={`${item}-${i}`} className="text-[13.5px] text-muted-foreground">
                      {item}
                    </div>
                  ))}
                </div>
...
function CheckList({ title, checks }: { title: string; checks: Purchase["checks"] }) {
  if (!checks.length) return null;
  return (
    <>
      <h3 className="mt-6 mb-3 font-heading text-[15px] font-medium text-muted-foreground">{title}</h3>
